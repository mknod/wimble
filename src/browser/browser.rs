...
//use wimble::browser_interface::BrowserInterface;
// import browser_interface::BrowserInterface;
//use self::BrowserInterface;
...
impl BrowserInterface for Browser {
    async fn goto(&self, url: &str) -> Result<(), String> {
        self.goto(url).await.map_err(|e| format!("{:?}", e))
    }

    async fn fetch_named_elements(&self, config: &BrowserConfig) -> Result<HashMap<String, String>, String> {
        self.get_named_elements(config).await.map_err(|e| format!("{:?}", e))
    }
}
 */
